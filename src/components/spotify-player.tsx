...
export default function SpotifyPlayer() {
  ...
  return (
    <section id="musica" ref={ref} className="max-w-3xl mx-auto px-3 sm:px-4 relative z-10">
      <div className={`css-fade-up ${isInView ? 'visible' : ''} glass-card rounded-[1.5rem] sm:rounded-[2rem] md:rounded-[4rem] p-5 sm:p-8 md:p-16 text-center relative overflow-hidden`}>
        ...
        <p className={`css-fade ${isInView ? 'visible' : ''} text-gray-400 italic mb-4 sm:mb-6 md:mb-8 text-xs sm:text-sm md:text-base px-2`} style={{ transitionDelay: '0.2s' }}>
        ...
        <a
          ...
          className={`css-fade-up ${isInView ? 'visible' : ''} inline-flex items-center gap-2 sm:gap-2.5 bg-[#1DB954] hover:bg-[#1ed760] text-white text-xs sm:text-sm md:text-base font-semibold px-5 sm:px-7 md:px-10 py-2.5 sm:py-3 md:py-3.5 rounded-full mb-6 sm:mb-10 md:mb-12 transition-all duration-300 shadow-lg active:scale-95`}
          style={{ transitionDelay: '0.3s', boxShadow: '0 4px 20px rgba(29,185,84,0.35)' }}
        >
        ...
        <p className={`css-fade ${isInView ? 'visible' : ''} mt-5 sm:mt-6 text-xs sm:text-sm md:text-base uppercase tracking-[0.15em] sm:tracking-[0.2em] text-goldLight/70 font-light`} style={{ transitionDelay: '0.5s' }}>
