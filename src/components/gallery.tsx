...
export default function Gallery() {
  ...
  const handleUpload = useCallback(async () => {
    ...
    try {
      ...
      if (res.ok) {
        // Add uploaded photo to local state immediately using preview URL
        ...
        try {
          ...
          if (serverRes.ok) {
            ...
            if (Array.isArray(data.photos) && data.photos.length > 0) {
              ...
              setPhotos(prev => {
                ...
                if (idx !== -1) {
                  updated[idx] = { ...updated[idx], src: latestUrl }
                }
                return updated
              })
            }
          ...
          // silently ignore - local preview is already showing
        }
        setTimeout(() => {
      ...
      >
        ...
        <div className="grid grid-cols-2 gap-2 sm:gap-3 md:gap-5">
          {photos.map((photo, idx) => {
            ...
            return (
              <motion.div
                ...
                viewport={{ once: true }}
                transition={{ duration: 0.6, delay: Math.min(idx, 7) * 0.1 }}
          ...
          <motion.div
            ...
            whileInView={{ opacity: 1, scale: 1 }}
            viewport={{ once: true }}
            transition={{ duration: 0.6, delay: Math.min(photos.length, 7) * 0.1 }}
            className="relative group cursor-pointer overflow-hidden rounded-lg sm:rounded-xl md:rounded-2xl aspect-[3/4] border-2 border-dashed hover:border-gold/40 transition-all duration-500 flex flex-col items-center justify-center gap-2 sm:gap-3"
      ...
      <AnimatePresence>
        {uploadModal && (
          <motion.div
            ...
            onClick={() => {
              if (uploadState === 'idle' || uploadState === 'ok' || uploadState === 'error') {
                setUploadModal(false)
                setSelectedFile(null)
                setPreview(null)
                setUploadState('idle')
          ...
          >
            ...
            >
              ...
              {!preview ? (
                ...
                >
                  <div className="w-14 h-14 rounded-full flex items-center justify-center text-gold"
                ...
                <div className="space-y-4">
                  ...
                  <div className="flex gap-3">
                    ...
                    >
                      {uploadState === 'ok' ? '¡Subida!' : 'Subir foto'}
                    </button>
                  </div>
                  ...
                  {(uploadState === 'compressing' || uploadState === 'uploading' || uploadState === 'ok' || uploadState === 'error') && (
                    <motion.div
                      ...
                      className={`flex items-center justify-center gap-2 text-xs ${statusConfig[uploadState].color}`}
                    >
                      {(() => {
                        const cfg = statusConfig[uploadState]
                        const Icon = cfg.icon
                        return <Icon size={14} className={cfg.spin ? 'animate-spin' : ''} />
                      })()}
