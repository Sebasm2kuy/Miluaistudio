            ...
            {TIMELINE.map((item, idx) => (
              ...
              >
                ...
                <div className="absolute left-5 sm:left-6 md:left-1/2 -translate-x-1/2 top-1 z-10">
                  <div
                    className="w-3 h-3 sm:w-3.5 sm:h-3.5 rounded-full border-2 border-goldLight bg-white"
                    style={{ boxShadow: '0 0 10px rgba(212,175,55,0.3)' }}
                  />
                </div>

                {/* Contenido */}
                <div className={`ml-12 sm:ml-14 md:ml-0 md:w-[45%] ${
                  idx % 2 === 0 ? 'md:pr-8 md:text-right' : 'md:pl-8 md:text-left md:ml-auto'
                }`}>
                  <div className={`inline-block px-4 sm:px-5 py-3 sm:py-4 md:py-5 rounded-2xl sm:rounded-2xl md:rounded-3xl border text-left ${
                  ...
                  >
                    <div className="flex items-center gap-2 sm:gap-3 mb-1">
                      ...
                      <span
                        className="font-bold text-bordeaux tabular-nums"
                        style={{ fontSize: 'clamp(0.85rem, 3vw, 1.4rem)' }}
                      >
                        {item.time} hs
                      </span>
                    </div>
                    <p className="font-serif italic text-bordeaux text-sm sm:text-base md:text-lg">
                      {item.title}
                    </p>
                    <p className="text-gray-400 text-[10px] sm:text-xs md:text-sm mt-0.5 uppercase tracking-widest">
                      {item.desc}
                    </p>
                  </div>
                </div>
